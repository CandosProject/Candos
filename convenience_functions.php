...
function openDB()
{
    try
    {
        ...
        $dbh = new PDO("mysql:host=localhost;dbname=dicom2;charset=utf8",'root','', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION)); // A changer suivant serveur local 
        return $dbh;
    }
...
function writeComment($db, $value){writeToDb($db, "Comment","examen_comment", $value);}
...
function printRadioButton($id,$label,$buttons,$required)
{
    ...
    foreach($buttons as $value => $buttonLabel)
    {
        ...
        if($required)
            echo ' required';
        echo ' />'."\n";
        echo '<label for="'.$id.'-'.$value.'" class="radio">'.$buttonLabel.'</label>'."\n";
    }
    echo '<br />';
}
...
{
    echo '<label for="'.$id.'">'.htmlentities($label).' :</label>'."\n";
    echo '<input type="text" id="'.$id.'" name="'.$id.'" class="add_admin" list="liste-'.$id.'" placeholder="'.strtolower($label).'"/>'."\n";
    ...
    echo '</datalist>'."\n";
    echo '<input type="submit" name="add-'.$id.'" value="Ajouter" class="add" />'."\n";
    echo '<br />'."\n";
}
...
function printDoubleAddableCombobox($id, $label, $placeholder1, $placeholder2, $contents)
{
    ...
    //text 1
    echo '<input type="text" class="add_admin2" id="'.$id.'-1" name="'.$id.'-1" placeholder="'.htmlentities($placeholder1).'"';
    disable();
    echo ' />';
    //text 2
    echo '<input type="text"  class="add_admin2" id="'.$id.'-2" name="'.$id.'-2" placeholder="'.htmlentities($placeholder2).'"';
    disable();
    echo ' />';
    //submit
    echo '<input type="submit" name="add-'.$id.'" value="Ajouter" class="add2"/>'."\n";
    echo '<br />'."\n";
}
...
{
    echo '<input type="submit" name="'.$name.'" value="'.htmlentities($value).'" class="bouton" ';
    disable();
    echo '/>';
}
